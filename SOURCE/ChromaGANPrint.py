...
def reconstruct(batchX, predictedY, filelist, epoch, i):
    ...
    save_path = os.path.join(save_models_path, filelist[i][:-4] +  "Epoch%dreconstructed.jpg" % epoch)
...
# Functions need names or Keras will throw an error
        ...
        for epoch in range(config.NUM_EPOCHS):
                for batch in range(int(data.size/config.BATCH_SIZE)):
                    ...
                    if (batch+1)%1000 ==0: 
                        ...
                        self.combined.save(save_path)  # creates a HDF5 file 'my_model.h5'
                        save_path = os.path.join(save_models_path, "my_model_colorization33Epoch%d_it%dWeights.h5" % (epoch, it))
                        self.colorizationModel.save(save_path)  # creates a HDF5 file 'my_model.h5'
                        save_path = os.path.join(save_models_path, "my_model_discriminator33Epoch%d_it%dWeights.h5" % (epoch, it))
                        self.discriminator.save_weights(save_path)  # creates a HDF5 file 'my_model.h5'
                ...
                save_path = os.path.join(save_models_path, "my_model_combined33Epoch%dWeights.h5" % epoch)
                self.combined.save_weights(save_path)  # creates a HDF5 file 'my_model.h5'
                save_path = os.path.join(save_models_path, "my_model_colorization33Epoch%dWeights.h5" % epoch)
                self.colorizationModel.save_weights(save_path)  # creates a HDF5 file 'my_model.h5'
                save_path = os.path.join(save_models_path, "my_model_discriminator33Epoch%dWeights.h5" % epoch)
                self.discriminator.save_weights(save_path)  # creates a HDF5 file 'my_model.h5'
    ...
    def sample_images(self, epoch):
        ...
        save_models_path = os.path.join(config.OUT_DIR,config.TEST_NAME)
        save_path = os.path.join(save_models_path, "my_model_colorization33Epoch0_it5000.h5")
        self.colorizationModel = load_model(save_path)#, custom_objects={'PConv2D': PConv2D, 'loss':self.loss_total(inputs_mask), 'PSNR': self.PSNR})
